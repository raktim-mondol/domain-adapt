...
class BMADataset(Dataset):
    ...
    Supports unlabeled data for unsupervised/semi-supervised domain adaptation.
    """
    ...
    def __init__(self, image_data_list, image_dir, augmentation=None, is_training=True,
        ...
        Args:
            ...
            is_labeled: Whether this dataset has labels (False for unsupervised target domain)
        """
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            ...
            label: int - pile-level label (same for all patches, including all augmented versions)
                   -1 if unlabeled (is_labeled=False)
        ...
        # Get label if labeled, otherwise use -1 as placeholder for unlabeled
        if self.is_labeled:
            label = image_data['pile_label']
        else:
            label = -1  # Sentinel value for unlabeled data
...
def create_bag_dataset_from_piles(df, pile_ids, image_dir, augmentation=None, is_training=True,
                                  ...
                                  num_augmentation_versions=1, is_labeled=True):
    """
    ...
    Args:
        ...
        num_augmentation_versions: Number of augmentation versions
        is_labeled: Whether this dataset has labels (for unsupervised/semi-supervised)
    ...
    for pile_id in pile_ids:
        ...
        # Get pile label (same for all images in pile) - only if labeled
        if is_labeled:
            pile_label = pile_images['BMA_label'].iloc[0] - 1  # Convert to 0-indexed
        else:
            pile_label = -1  # Placeholder for unlabeled data
    ...
    return BMADataset(image_data_list, image_dir, augmentation, is_training,
                     include_original_and_augmented, num_augmentation_versions, is_labeled)
