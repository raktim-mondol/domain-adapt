"""
Minimal test script for new data split configuration functionality
Tests both percentage-based and per-class count-based splitting (standalone version)
"""
...
# Import only the data split utilities (avoid importing torch dependencies)
from src.utils.data_split_utils import split_domain_data, validate_split
...
def test_per_class_count_split():
    ...
    if is_valid and all_match:
        print("\n✓ TEST 2 PASSED: Per-class count split is valid and accurate!")
    else:
        print("\n✗ TEST 2 FAILED: Per-class count split has issues!")

    return is_valid and all_match
...
def main():
    """Run all tests"""
    print("\n" + "="*80)
    print("DATA SPLIT CONFIGURATION - TEST SUITE (MINIMAL)")
    print("="*80)
